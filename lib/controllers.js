...
/* 我的课程-月历视图 - controller */
ScheduleMonthController = RouteController.extend({
  getYear: function() {
    var monthStr = this.params.monthStr, year = null;
    if (monthStr) {
      var a = monthStr.split('-');
      year = parseInt(a[0]);
    }
    if (isNaN(year) || !year || year < 2015) {
      year = new Date().getFullYear();
    }
    return year;
  },
  getMonth: function() {
    var monthStr = this.params.monthStr, month = null;
    if (monthStr) {
      var a = monthStr.split('-');
      month = parseInt(a[1]);
    }
  ...
  findOptions: function() {
    ...
    var targetMonth = new Date(year, month, 1), startTime = targetMonth.getTime(), endTime = startTime + 31*ScheduleTable.MS_PER_DAY;
  ...
  action: function() {
    ...
    var role = Meteor.user().role;
    if (role==='teacher') {
      this.render('scheduleMonthTeacher');
    } else {
      this.render('scheduleMonthStudent')
    }
  }
});