...
TeachersController = RouteController.extend({
  ...
  waitOn: function() {
    ...
    if(way == "studyCenter"){
      subscriptionTerms.find = _.extend(subscriptionTerms.find, {'profile.studyCenters': {"$in": studyCenters}});
    }
    if(limit){
  ...
  onAfterAction: function () {
    var teachers = this.teachers(this.parameters());
    if (!teachers) return;
    var a = [];
    ...
    a = _.uniq(a);
    Meteor.subscribe('teacherPrices', a);
  }
});
