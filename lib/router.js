...
Router.route('/about', { name: 'about', template: 'pageDetail',
  ...
  data: function() {
    var clm = Pages.findOne({pageCode: 'about'});
    if(!clm){
      return new Object();
    }
    return clm;
...
Router.route('/rights', { name: 'rights', template: 'pageDetail',
  ...
  data: function() {
    var clm = Pages.findOne({pageCode: 'rights'});
    if(!clm){
      return new Object();
    }
    return clm;
...
Router.route('/page/edit/:_pageCode', { name: 'pageEdit',
  waitOn: function() {
    return [
      Meteor.subscribe('pages')
    ];
  ...
  data: function() {
    if(!this.params._pageCode){
      return new Object();
    }
    var clm = Pages.findOne({pageCode: this.params._pageCode});
    if(!clm){
      var obj = new Object();
      obj.pageCode = this.params._pageCode;
      return obj;
    }
    return clm;
  }
});
Router.route('/page/:_pageCode', { name: 'pageDetail',
  waitOn: function() {
    return [
      Meteor.subscribe('pages')
    ];
