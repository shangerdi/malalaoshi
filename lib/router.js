Router.configure({
  layoutTemplate: function() {
    ...
    if (Meteor.isCordova) {
      ...
      if (_.contains(['teachers', 'orders', 'order'], curRouteName)) {
        ans = 'appTabsLayout';
      }
      else {
        ans = 'appLayout';
    ...
    if(orderId){
      ...
      if(order){
        if(order.student && order.student.id){
          student = Meteor.users.findOne(order.student.id);
        }
        if(order.teacher && order.teacher.id){
          teacher = Meteor.users.findOne(order.teacher.id);
        }
      }
    }else{
...
Router.route('/parentSettings', {name: 'parentSettings'});
Router.route('/parentSettingsEdit', {name: 'parentSettingsEdit'});
