...
Router.route('/schedule/weekly', {name: 'scheduleWeekly',
  waitOn:function() {
    ...
    return [
      Meteor.subscribe('teacherCourseTable', Meteor.userId()),
      Meteor.subscribe('areaTimePhases', code)
    ];
  }
});
Router.route('/schedule/daily', function(){
  var role = Meteor.user().role;
  if (role==='teacher') {
    this.render('scheduleDailyTeacher');
  } else {
    this.render('scheduleDailyStudent')
  }
}, {name: 'scheduleDaily'});
Router.route('/schedule/weekly/for/order/test/:teacherId?', {name: 'scheduleWeeklyForOrder',
  waitOn:function() {
    if (!Meteor.userId()) {
      return null;
    }
    var teacherId = this.params.teacherId, teacher = Meteor.users.findOne({"_id": teacherId, role: 'teacher'});
    if (teacher) {
      var address = teacher.profile.address, code=[], attendanceQuery = {find:{"teacher.id":teacherId},options:{}};
      if (address.province.code) {
        code.push(address.province.code);
      }
      if (address.city.code) {
      ...
      var now = new Date(), todayTime = new Date(now.getFullYear(),now.getMonth(),now.getDate()).getTime(), laterTime = todayTime+(7+CourseTable.experienceDays)*24*60*60*1000;
      attendanceQuery.find.attendDay = {$gte: todayTime, $lt: laterTime};
    } else {
      return null;
    }
    return [
      Meteor.subscribe('teacherCourseTable', teacherId),
      Meteor.subscribe('areaTimePhases', code),
      Meteor.subscribe('courseAttendanceList', attendanceQuery)
    ];
  }
});
